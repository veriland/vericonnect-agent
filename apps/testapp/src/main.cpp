...
#if defined(_WIN32)
#include <windows.h>
BOOL WINAPI ctrl_handler(DWORD) { g_stop = true; return TRUE; }
void install_ctrl_handler() { SetConsoleCtrlHandler(ctrl_handler, TRUE); }
#else
...
std::optional<std::string> build_command_json(int argc, char **argv)
{
    ...
    if (const char *v = arg_value(argc, argv, "--folder"))      params.set("TargetFolder", vc::Json::string(v));
    if (const char *v = arg_value(argc, argv, "--file"))        params.set("FileName", vc::Json::string(v));
    if (const char *v = arg_value(argc, argv, "--content"))     params.set("FileContent", vc::Json::string(v));
    if (const char *v = arg_value(argc, argv, "--dest-folder")) params.set("DestinationFolder", vc::Json::string(v));
    if (const char *v = arg_value(argc, argv, "--dest-file"))   params.set("DestinationFileName", vc::Json::string(v));
...
{
    if (body.empty()) { std::printf("  (empty body)\n"); return; }
...
int do_send(int argc, char **argv)
{
    ...
    std::string token = vc::sas_token(settings.relay.namespace_host,
